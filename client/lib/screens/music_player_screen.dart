...
import '../audio_manager.dart'; // 오디오 본부 연결
import 'lyrics_screen.dart';

class MusicPlayerScreen extends StatefulWidget {
  const MusicPlayerScreen({super.key}); // 인자 필요 없음
...
class _MusicPlayerScreenState extends State<MusicPlayerScreen> {
  ...
  final ValueNotifier<List<Map<String, dynamic>>> _lyricsNotifier = ValueNotifier([]);
  ...
  final Map<String, String> _languages = {"자동 감지": "auto", "한국어": "ko", "영어": "en", "일본어": "ja"};
  ...
  void _updateCurrentSongInfo(int index) {
    if (AudioManager().currentPlaylist.isEmpty || index >= AudioManager().currentPlaylist.length) return;
  ...
  void _updateCurrentLyricLine(Duration position) {
    ...
    var match = _lyricsNotifier.value.lastWhere((line) => (line['start'] as num).toDouble() <= currentSeconds, orElse: () => {});
    if (match.isNotEmpty && match['text'] != _currentLyricLine) {
      setState(() => _currentLyricLine = match['text']);
    }
  }

  void _playPrev() {
    if (_audioPlayer.hasPrevious) _audioPlayer.seekToPrevious();
    else _audioPlayer.seek(Duration.zero, index: AudioManager().currentPlaylist.length - 1);
  }
  ...
  void _showManualLyricsDialog() {
    TextEditingController controller = TextEditingController(text: _manualLyrics);
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: TextField(
            ...
            decoration: const InputDecoration(hintText: "가사를 입력하세요", border: OutlineInputBorder()),
          ),
        ),
        actions: [
            ...
            return AlertDialog(
              ...
              content: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  value: tempLanguage, isExpanded: true,
  ...
  Future<void> _requestAiLyrics() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> newLyrics = List<Map<String, dynamic>>.from(json['segments']);
        _lyricsNotifier.value = newLyrics;
        _progressNotifier.value = 1.0;
        globalLyricsCache[song.id] = newLyrics;
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("가사 생성 완료!")));
      }
    } catch (e) {
      ...
      builder: (context, currentIndex, child) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            leading: IconButton(icon: const Icon(Icons.keyboard_arrow_down, size: 30), onPressed: () => Navigator.pop(context)),
          ),
          body: LayoutBuilder(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(border: Border.all(color: Colors.grey), color: kAppGrey),
                          child: QueryArtworkWidget(id: song.id, type: ArtworkType.AUDIO, nullArtworkWidget: const Icon(Icons.music_note, size: 80)),
                        ),
                        const SizedBox(height: 20),
                        // 2. 제목/가수
                        Column(children: [
                          Text(song.title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
                        ...
                        GestureDetector(
                          onTap: () {
                             Navigator.push(context, MaterialPageRoute(builder: (context) => LyricsFullScreen(
                               allSongs: AudioManager().currentPlaylist, 
                               audioPlayer: _audioPlayer,
                               currentIndexNotifier: _currentIndexNotifier,
                               lyricsNotifier: _lyricsNotifier,
                               onPrev: _playPrev, onNext: _playNext, onPlayPause: _togglePlay, onShowAiModal: _showAiLanguageDialog,
                             )));
                          },
                          child: Container(
                            width: double.infinity, padding: const EdgeInsets.all(15),
                            decoration: BoxDecoration(color: kAppGrey, border: Border.all(color: Colors.grey)),
                            child: Text(_currentLyricLine, textAlign: TextAlign.center, maxLines: 2),
                          ),
                        ),
                        const SizedBox(height: 20),
                        // 4. 슬라이더
                        StreamBuilder<Duration>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            return Column(
                              children: [
                                Slider(
                                  activeColor: kAppBlack, inactiveColor: Colors.grey, thumbColor: kAppBlack,
                                  min: 0, max: total.inSeconds.toDouble() > 0 ? total.inSeconds.toDouble() : 1.0,
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      Text(total.toString().split('.').first.padLeft(8, "0").substring(3)),
                                    ],
                                  ),
                                ),
                              ],
                        ...
                        Row(
                          ...
                          children: [
                            IconButton(icon: const Icon(Icons.arrow_back, size: 40), onPressed: _playPrev),
                            Container(
                              decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: kAppBlack)),
                              child: IconButton(icon: Icon(_isPlaying ? Icons.pause : Icons.play_arrow, size: 40), onPressed: _togglePlay),
                            ),
                            IconButton(icon: const Icon(Icons.arrow_forward, size: 40), onPressed: _playNext),
                          ],
                        ),
                        const SizedBox(height: 20),
                        // 6. 하단 버튼
                        ValueListenableBuilder<bool>(
                          ...
                          builder: (context, isAnalyzing, child) {
                            return Row(
                              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                              children: [
                                ElevatedButton(onPressed: _showManualLyricsDialog, style: ElevatedButton.styleFrom(backgroundColor: kAppGrey), child: const Text("가사 넣기")),
                                if (isAnalyzing) const CircularProgressIndicator()
                                else ElevatedButton(onPressed: _showAiLanguageDialog, child: const Text("AI 가사 생성")),
                              ],
                            );
                          },
                        )
